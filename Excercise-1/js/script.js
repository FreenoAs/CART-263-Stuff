...
function ball() {//ball bounces around the screen 
    ...
    if (ballY == 0) {
        ...
        ballX = width / 2;
        ballY = height / 2;
    } if (ballX == xposP1+40 && ballY >= yposP1 && ballY >= yposP1) {//bounces off the left and right (the second part is to only bounce off when it hits the players but i cant seem to get that collosion working)
        movementX = -1; 
    } if (ballX == xposP2 && ballY >= yposP2 && ballY >= yposP2) {
        movementX = 1;
    }
}